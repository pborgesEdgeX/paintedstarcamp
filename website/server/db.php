...
// Add Customer to DB
$sql = "INSERT INTO customers (parentName, lastName, email, phone, studentName, carrier, age, week, email_sent, text_sent, waitlisted, payment)
VALUES ('$parent_name', '$last_name','$email', '$phone', '$student_name', '$carrier', '$age', '$week', '$email_sent', '$text_sent', '$waitlisted', '$payment')";
...
if ($conn->query($sql) === TRUE) {
    if(mail($to, $subject, $message, $headers)):
        ...
        if ($conn->query($sql) === TRUE)
        {
          if(mail($to_txt, $subject, $parent_name.", Thank you for Signing up for Camp! Your spot is NOT secured yet. Please follow the instructions sent to: ".$email, "From: Shannen Cullinan <shannen@paintedstarequestrian.com>\r\n"))
          {
                //echo 'Text Sent';
                $sql = "UPDATE customers SET text_sent='1' WHERE email='$email'";
                if ($conn->query($sql) === TRUE)
                  {
                    // Text Admin of the website once record is updated
                    mail($admin_number.'@vtext.com', $admin_subject, "\n\n".$Text, "From:".$email);
                  }
                else
                  {
